...
whatever worktree the pane fronts now — tmux recycles `%N` — and the pane
...
# --- a tmux server that only records ---


@dataclass
class FakeTmux:
    """Stands in for `events._tmux` / `_tmux_out`, so pane options and wait-for
    signals can be inspected without a tmux server."""
    ...
    def tmux(self, socket: str, *args: str) -> None:
...
def tmux(monkeypatch):
    ...
    monkeypatch.setattr(events, "_tmux", fake.tmux)
...
def probe(tmp_path, db_path, tmux):
    ...
    try:
        yield Probe(handle=handle, db=db_path, tmux=tmux)
    finally:
        handle.stop()
...
def test_an_event_is_recorded_and_attributed_to_the_caller(probe, crane):
    status, payload = probe.post("/v1/events", {"kind": "busy", "detail": "Edit"}, crane)
    assert status == 200
    event = payload["event"]
    ...
    assert event["worktree_id"] == probe.worktrees["swift-crane"]
    assert payload["cursor"] == event["id"]

    stored = store.iter_events(pane="%1", db_path=probe.db)
    assert len(stored) == 1
...
def test_the_pane_option_is_set_on_the_socket_the_row_names(probe):
    token = probe.agent("%1", "swift-crane", socket_name="amux-other")
    probe.post("/v1/events", {"kind": "notify", "detail": "may I?"}, token)
    assert probe.tmux.options() == [("amux-other", "%1", "needs-input")]
    assert probe.tmux.signals() == [("amux-other", "amux-state-1")]
...
def test_every_event_kind_maps_to_the_native_state(probe, crane, kind, state):
    _, payload = probe.post("/v1/events", {"kind": kind}, crane)
    assert payload["event"]["state"] == state
    assert probe.tmux.options()[-1] == ("amux-root", "%1", state)
    assert events.STATE_BY_KIND[kind] == state


def test_a_needs_input_event_resolves_the_pane_to_needs_input(probe, crane):
    """The scenario from the spec: a hook posts a notification, and the host
    pane resolves to `needs-input`."""
    probe.post("/v1/events", {"kind": "notify", "detail": "which branch?"}, crane)
    assert probe.tmux.options()[-1][2] == "needs-input"
    state = events.resolve_state(
        alive=True,
        option="needs-input",
        latest=events.Event.from_row(store.latest_event("%1", db_path=probe.db)),
    )
    assert state == "needs-input"


def test_an_event_stays_attributed_after_the_pane_id_is_recycled(probe):
    """tmux hands `%N` back out after a restart. `events.emit` asks which
    ...
    old_token = probe.agent("%1", "old-agent")
    old_id = probe.worktrees["old-agent"]
    ...
    assert store.worktree_for_pane("%1", db_path=probe.db)["id"] == new_id

    _, payload = probe.post("/v1/events", {"kind": "busy"}, old_token)
    assert payload["event"]["worktree_id"] == old_id
    assert [e["worktree_id"] for e in store.iter_events(pane="%1", db_path=probe.db)] == [
        old_id
    ]


def test_an_event_for_a_vanished_pane_is_still_recorded(probe):
    """A killed task with a live sandbox: the event is durable, and the response
    says plainly that nothing was signalled."""
    token = probe.agent("%99", "ghost")
    _, payload = probe.post("/v1/events", {"kind": "stop"}, token)
    assert payload["pane_updated"] is False
    assert probe.tmux.options() == []
    assert probe.tmux.signals() == []
    assert len(store.iter_events(pane="%99", db_path=probe.db)) == 1


def test_a_body_cannot_attribute_an_event_to_another_pane(probe, crane):
    other = probe.agent("%2", "happy-deer")
    _, payload = probe.post(
        "/v1/events",
        {
            "kind": "exit",
            "pane": "%2",
    ...
    assert payload["event"]["worktree_id"] == probe.worktrees["swift-crane"]
    assert probe.tmux.options()[-1][1] == "%1"


@pytest.mark.parametrize("kind", ["", "restart", "BUSY", "busy ", None, 7])
def test_an_unknown_event_kind_is_refused(probe, crane, kind):
    status, payload = probe.post("/v1/events", {"kind": kind}, crane)
    assert status == 400
    assert payload["error"]["code"] == "invalid_request"
    assert store.iter_events(db_path=probe.db) == []
    assert probe.tmux.options() == []


def test_an_oversized_detail_is_refused_with_both_numbers(probe, crane):
    limit = probe.handle.service.config.max_detail_chars
...
def test_an_empty_detail_is_fine(probe, crane):
    _, payload = probe.post("/v1/events", {"kind": "stop"}, crane)
    assert payload["event"]["detail"] == ""


def test_posting_an_event_needs_the_write_capability(probe):
    token = probe.agent("%1", "reader", permissions=(cs.PERM_CONTEXT_READ,))
    status, payload = probe.post("/v1/events", {"kind": "busy"}, token)
    assert status == 403
...
def test_state_reports_the_panes_in_the_callers_workspace(probe, crane):
    probe.tmux.listing = [
        _pane_line("%1", "swift-crane", "proj", "fix", "busy"),
        _pane_line("%2", "happy-deer", "proj", "fix", "idle"),
    ]
    status, payload = probe.get("/v1/events/state", crane)
    assert status == 200
...
def test_state_hides_another_workspace(probe, crane):
    probe.tmux.listing = [
        _pane_line("%1", "swift-crane", "proj", "fix", "busy"),
        _pane_line("%7", "someone-else", "other-project", "task0", "idle"),
    ]
    _, payload = probe.get("/v1/events/state", crane)
    assert [p["pane"] for p in payload["panes"]] == ["%1"]
    assert "other-project" not in json.dumps(payload)


def test_state_needs_the_read_capability(probe):
    token = probe.agent("%1", "writer", permissions=(cs.PERM_EVENTS_WRITE,))
    status, payload = probe.get("/v1/events/state", token)
    assert status == 403
...
def test_wait_returns_at_once_when_the_pane_is_already_there(probe, crane):
    probe.tmux.facts["%1"] = events.PaneFacts(
        alive=True, kind="amux", created=1000.0, state_option="idle"
    )
    started = time.monotonic()
    status, payload = probe.get("/v1/events/wait?pane=%251&timeout=5", crane)
    assert status == 200
...
def test_wait_is_released_by_a_sandbox_event(probe, crane):
...
def test_wait_sees_a_native_write_too(probe, crane):
    ...
    store.add_event(
        ...
        workspace="proj",
        task="fix",
        worktree_id=probe.worktrees["swift-crane"],
        repo="/repos/proj",
        db_path=probe.db,
    )
    probe.tmux.facts["%1"] = events.PaneFacts(
...
def test_wait_expires_with_a_usable_cursor(probe, crane):
    probe.tmux.facts["%1"] = events.PaneFacts(
        alive=True, kind="amux", created=1000.0, state_option="busy"
    )
    _, first = probe.post("/v1/events", {"kind": "busy"}, crane)
    started = time.monotonic()
...
def test_wait_never_exceeds_the_configured_cap(probe, crane):
    probe.tmux.facts["%1"] = events.PaneFacts(
        alive=True, kind="amux", created=1000.0, state_option="busy"
    )
    started = time.monotonic()
    ...
    assert status == 200
    assert payload["state"] is None
    assert elapsed < probe.handle.service.config.max_wait_s + 2


def test_wait_resumes_from_a_cursor_without_repeating(probe, crane):
    probe.tmux.facts["%1"] = events.PaneFacts(
        alive=True, kind="amux", created=1000.0, state_option="idle"
    )
    _, first = probe.post("/v1/events", {"kind": "busy"}, crane)
    _, second = probe.post("/v1/events", {"kind": "stop"}, crane)

    _, payload = probe.get("/v1/events/wait?pane=%251&timeout=1", crane)
...
def test_wait_events_are_in_identifier_order(probe, crane):
    probe.tmux.facts["%1"] = events.PaneFacts(
        alive=True, kind="amux", created=1000.0, state_option="idle"
    )
    for kind in ("spawn", "busy", "stop", "busy", "stop"):
        probe.post("/v1/events", {"kind": kind}, crane)
    _, payload = probe.get("/v1/events/wait?pane=%251&timeout=1&after=0", crane)
    ids = [e["id"] for e in payload["events"]]
    assert ids == sorted(ids)


def test_wait_defaults_to_the_callers_own_pane(probe, crane):
    probe.tmux.facts["%1"] = events.PaneFacts(
        alive=True, kind="amux", created=1000.0, state_option="idle"
    )
    _, payload = probe.get("/v1/events/wait?timeout=1", crane)
    assert payload["pane"] == "%1"
    assert payload["state"] == "idle"


def test_wait_allows_a_teammate_in_the_same_scope(probe, crane):
    probe.agent("%2", "happy-deer")
    probe.tmux.facts["%2"] = events.PaneFacts(
        alive=True, kind="amux", created=1000.0, state_option="idle"
    )
    _, payload = probe.get("/v1/events/wait?pane=%252&timeout=1", crane)
    assert payload["pane"] == "%2"
    assert payload["state"] == "idle"


def test_wait_refuses_a_pane_outside_the_scope(probe, crane):
    ...
    for pane in ("%9", "%404"):
        status, payload = probe.get(f"/v1/events/wait?pane=%25{pane[1:]}&timeout=1", crane)
        assert status == 403, pane
        assert payload["error"]["code"] == "forbidden"
        assert payload["error"]["message"] == f"pane {pane} is not in proj/fix"


def test_wait_refuses_a_pane_in_another_repository(probe, crane):
    store.register_worktree(
        pane="%5",
        workspace="proj",
        task="fix",
    ...
    status, payload = probe.get("/v1/events/wait?pane=%255&timeout=1", crane)
    assert status == 403
...
def test_out_of_bounds_wait_parameters_are_refused(probe, crane, query):
    status, payload = probe.get(f"/v1/events/wait?{query}", crane)
    assert status in (400, 403)
    assert payload["error"]["code"] in ("invalid_request", "forbidden")


def test_an_empty_parameter_means_unspecified(probe, crane):
    ...
    for query in ("states=", "limit=", "after=", "timeout=", "pane="):
        status, payload = probe.get(f"/v1/events/wait?timeout=1&{query}", crane)
        assert status == 200, query
        assert payload["pane"] == "%1"


def test_wait_accepts_every_state_name(probe, crane):
    probe.tmux.facts["%1"] = events.PaneFacts(
        alive=True, kind="amux", created=1000.0, state_option="idle"
    )
    _, payload = probe.get(
...
def test_the_default_wait_states_match_the_native_ones(probe, crane):
    assert cs.DEFAULT_WAIT_STATES == ("idle", "needs-input", "dead")
    assert set(cs.DEFAULT_WAIT_STATES) <= set(cs.AGENT_STATES)


def test_waiting_needs_the_read_capability(probe):
    token = probe.agent("%1", "writer", permissions=(cs.PERM_EVENTS_WRITE,))
    status, payload = probe.get("/v1/events/wait?timeout=1", token)
    assert status == 403
    assert cs.PERM_CONTEXT_READ in payload["error"]["message"]
