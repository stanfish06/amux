...
def test_a_minted_token_authenticates_and_carries_the_whole_row(probe, spy):
    token, token_id, worktree_id = probe.agent()
    status, payload = probe.get(spy, token=token)
    assert status == 200
...
def test_identity_ignores_every_field_the_body_supplies(probe, spy):
    token, _, worktree_id = probe.agent()
...
def test_identity_ignores_every_field_the_query_supplies(probe, spy):
    token, _, worktree_id = probe.agent()
    _, payload = probe.get(
...
def test_two_agents_get_two_identities(probe, spy):
    first_token, _, first_id = probe.agent(pane="%1", name="swift-crane")
    second_token, _, second_id = probe.agent(pane="%2", name="happy-deer")
    _, first = probe.get(spy, token=first_token)
    _, second = probe.get(spy, token=second_token)
...
def test_one_agents_token_cannot_become_another(probe, spy):
    """The obvious attack: hold a valid capability, ask to be someone else."""
    victim_id = probe.register(pane="%2", name="happy-deer", agent="codex")
    attacker_token, _, attacker_id = probe.agent(pane="%1", name="swift-crane")
    _, payload = probe.post(
...
def test_the_socket_comes_from_the_row_so_events_reach_the_right_server(probe, spy):
    token, _, _ = probe.agent(socket_name="amux-other")
    _, payload = probe.get(spy, token=token)
    assert payload["identity"]["socket"] == "amux-other"


def test_a_row_without_a_socket_falls_back_to_the_service_socket(probe, spy):
    token, _, _ = probe.agent(socket_name="")
    _, payload = probe.get(spy, token=token)
    assert payload["identity"]["socket"] == probe.handle.service.config.socket


def test_a_host_runtime_row_with_a_capability_still_authenticates(probe, spy):
    """Nothing here gates on the runtime: a capability is a capability."""
    token, _, _ = probe.agent(
...
def test_an_unknown_token_is_rejected(probe, spy):
    probe.agent()  # a real capability exists, just not this one
    status, payload = probe.get(spy, token="not-a-real-token")
    assert status == 401
    assert payload["error"]["code"] == "unauthorized"


def test_a_revoked_token_is_rejected(probe, spy):
    token, token_id, _ = probe.agent()
    assert probe.get(spy, token=token)[0] == 200
    store.revoke_context_token(token_id, db_path=probe.db)
    status, payload = probe.get(spy, token=token)
    assert status == 401
    assert payload["error"]["code"] == "unauthorized"


def test_removing_a_sandbox_revokes_its_capabilities(probe, spy):
    token, _, worktree_id = probe.agent()
    other_token, _, _ = probe.agent(pane="%2")
    revoked = store.revoke_context_tokens_for_worktree(worktree_id, db_path=probe.db)
    assert revoked == 1
    assert probe.get(spy, token=token)[0] == 401
    # A teammate's capability is untouched.
    assert probe.get(spy, token=other_token)[0] == 200


def test_an_expired_token_is_rejected(probe, spy):
    worktree_id = probe.register()
    token, _ = probe.mint(worktree_id, now=time.time() - 100, ttl=1.0)
    status, payload = probe.get(spy, token=token)
    assert status == 401
    assert payload["error"]["code"] == "unauthorized"


def test_a_token_whose_execution_was_removed_is_rejected(probe, spy):
    """Removal is meant to revoke; if that failed, the row still refuses."""
    token, _, worktree_id = probe.agent()
    store.set_worktree_status(worktree_id, "removed", db_path=probe.db)
    status, payload = probe.get(spy, token=token)
    assert status == 401
    assert payload["error"]["code"] == "unauthorized"
...
def test_a_merged_execution_still_authenticates(probe, spy):
    ...
    token, _, worktree_id = probe.agent()
    store.set_worktree_status(worktree_id, "merged", db_path=probe.db)
    status, payload = probe.get(spy, token=token)
    assert status == 200
    assert payload["identity"]["status"] == "merged"


def test_unknown_expired_and_revoked_are_indistinguishable(probe, spy):
    """A caller must not be able to tell which of the three it hit — that is a
    guessing oracle."""
    worktree_id = probe.register()
    expired, _ = probe.mint(worktree_id, now=time.time() - 100, ttl=1.0)
    revoked, revoked_id = probe.mint(worktree_id)
    store.revoke_context_token(revoked_id, db_path=probe.db)
    answers = {
        json.dumps(probe.get(spy, token=t)) for t in ("unknown-token", expired, revoked)
    }
    assert len(answers) == 1


def test_a_token_from_another_database_does_not_authenticate(probe, spy, tmp_path):
    ...
    assert probe.get(spy, token=foreign)[0] == 401
...
def test_a_route_refuses_a_capability_that_lacks_its_permission(probe):
    cs._ROUTES[("POST", "/v1/needs-notes")] = cs.Route(
        handler=lambda service, request: (200, {"ok": True}),
        requires=cs.PERM_NOTES_WRITE,
    )
    try:
        allowed, _, _ = probe.agent(pane="%1", permissions=cs.AGENT_PERMISSIONS)
        read_only, _, _ = probe.agent(pane="%2", permissions=(cs.PERM_CONTEXT_READ,))
        assert probe.post("/v1/needs-notes", token=allowed)[0] == 200
        status, payload = probe.post("/v1/needs-notes", token=read_only)
        assert status == 403
...
def test_a_capability_with_no_permissions_can_reach_nothing_that_needs_one(probe):
    cs._ROUTES[("GET", "/v1/needs-read")] = cs.Route(
        handler=lambda service, request: (200, {"ok": True}),
        requires=cs.PERM_CONTEXT_READ,
    )
    try:
        token, _, _ = probe.agent(permissions=())
        assert probe.get("/v1/needs-read", token=token)[0] == 403
    finally:
        del cs._ROUTES[("GET", "/v1/needs-read")]


def test_permissions_round_trip_through_the_store(probe, spy):
    token, _, _ = probe.agent(permissions=(cs.PERM_EVENTS_WRITE,))
    _, payload = probe.get(spy, token=token)
...
def test_a_handler_cannot_forget_to_authenticate(probe):
...
def test_the_plaintext_token_is_absent_from_the_database(probe, spy):
    token, _, _ = probe.agent()
    assert probe.get(spy, token=token)[0] == 200
    assert token.encode() not in probe.db.read_bytes()
    for suffix in ("-wal", "-shm"):
        sidecar = probe.db.with_name(probe.db.name + suffix)
        if sidecar.exists():
            assert token.encode() not in sidecar.read_bytes()


def test_the_plaintext_token_is_absent_from_the_logs(probe, spy):
    token, _, worktree_id = probe.agent()
    probe.get(spy, token=token)
    probe.get(spy, token="wrong-" + token)
    joined = "\n".join(probe.messages)
    assert token not in joined
    assert probe.handle.service.config.log_file.read_text().find(token) == -1
    # The caller is still identifiable, by durable id.
    assert f"wt{worktree_id}" in joined
